...
suppressMessage(require(ggplot2))
...
hom.lm.train <- suppressWarnings(
    train(optimism~., method="lm", data=hom.train, trControl=ctrl))
train.totals <- dplyr::select(hom.train, 77:86)
hom.lm.train.totals <- train(optimism~., method="lm", data=train.totals, preProcess="scale", tuneLength=tl, trControl=ctrl)
...
hom.rf.train.totals <- suppressMessages(train(optimism~., method="rf", data=train.totals, preProcess="scale", tuneLength=tl, trControl=ctrl))
hom.glmnet.train.totals <- suppressMessages(train(optimism~., method="glmnet", data=train.totals, preProcess="scale", tuneLength=tl, trControl=ctrl))
@       
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.5\textwidth}
<<glmboost, echo=FALSE>>=
hom.rpart.train.totals <- train(optimism~., method="rpart", data=train.totals, preProcess="scale", tuneLength=tl, trControl=ctrl)
hom.glmboost.train.totals <- train(optimism~., method="glmboost", data=train.totals, preProcess=c("scale", "center"),  tuneLength=tl, metric="Rsquared", trControl=ctrl)
@       
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
